...
wx项目地址 https://lh100-1253806495.cos.ap-beijing.myqcloud.com/index.html?carteamflg=100036&extflg=30583
...
脚本第31行 const waittm = 25080   25080是等待25.08秒 可自己修改
可随意修改脚本  转发请留作者名
*/
const $ = new Env("蜗牛");
let envSplitor = ["@", "\n"];
...
class UserInfo {
   ...
   async getReadTaskListBaidu() {
      const body = `act=getReadTaskListBaidu&passport=${this.k}`;
      await httpRequest(method, popu(this.url, body));
      let result = httpResult;
      try {
         if (result?.ret == "succ") {
            this.id = result?.task_list[0]?.id;
            console.log(`${this.idx}初始ID获取成功 [${this.id}]`);
            this.f = 1;
         } else {
            console.log(
               `${this.idx}初始ID获取失败 原因 ${JSON.stringify(result.msg)} `
            );
         }
      } catch (error) {}
   }
   ...
   async userDrawReadTaskBaidu(a) {
      ...
      let result = httpResult;
      if (result?.code == 0) {
         console.log(`${this.idx}领取：ID${a}` + result.msg);
         this.id = result.data.user_task_id;
         result.msg == "任务领取成功" &&
            (await $.wait(waittm), await this.finishUserReadTaskBaidu());
      } else {
         console.log(`${this.idx}领取失败了：ID [${a}]` + result.msg);
         if (result?.msg == "全天百度推送已完成") {
            console.log(`${this.idx}已阅读完毕 跳出执行 原因:` + result.msg);
            this.bk = 1;
         }
      }
   }
   ...
   async task() {
      await this.getReadTaskListBaidu();
      if (this.f == 1) {
         await this.getUserMoneyAccounts();
         await this.finishAddCompanyWexinReward();
         console.log(`${this.idx}扫任务：扫到后运行领取 `);
         var x = this.id;
         for (let a of [...new Array(300).keys()]) {
            var c = Number(x) + Number(a);
            await this.userDrawReadTaskBaidu(c);
            if (this.bk == 1) break;
         }
      }
   }
